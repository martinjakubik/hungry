...
echo case $test_case
input="-o --when -3"
expected="usage: ./h_lib.sh --home -h [--days 3|-d 3] --office -o [-w | --what {Burrito, Ravioli, Burger, Pho, Sushi, Sushhhhiiiiiifdidiiiissqihiii}] --badminton -b [--when {Mo, Tu, We, Th, Sh, 1, 2, 3, ...}] --post_to_teams --mock_api --debug"
actual=$(./h --mock_date "2020-01-03" $input 2>&1)
run_count=$(( run_count+1 ))
if [[ ! "$actual" = "$expected" ]] ; then
    fail_count=$(( fail_count+1 ))
...
test_case="checking valid --when for tomorrow"
echo case $test_case
input="-o --when 1"
expected="12:00:00.001 in lobby tomorrow?"
actual=$(./h --mock_date "2020-01-03" $input 2>&1)
...
test_case="checking valid equal sign --when=1"
echo case $test_case
input="-o --when=1"
expected="12:00:00.001 in lobby tomorrow?"
actual=$(./h --mock_date "2020-01-03" $input 2>&1)
